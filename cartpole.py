...
def run(is_training=True, render=False):
    ...
    pos_space = np.linspace(-2.4, 2.4, 15)
    vel_space = np.linspace(-4, 4, 15)
    ang_space = np.linspace(-.2095, .2095, 15)
    ang_vel_space = np.linspace(-4, 4, 15)

    if(is_training):
        Q_table = np.zeros((len(pos_space)+1, len(vel_space)+1, len(ang_space)+1, len(ang_vel_space)+1, env.action_space.n)) # init a 11x11x11x11x2 array
    ...
    plt.plot(mean_rewards, label="Mean Rewards (Last 100 Episodes)")
    plt.xlabel("Episode")
    plt.ylabel("Mean Reward")
    plt.title("Performance Over Episodes - CartPole")
    plt.legend()
    plt.grid()
